...
function LinkButton({ children, to, type = '', onClick, className }) {
  const defaultStyles = {
    primary:
    ...
    nav2: 'flex gap-2 items-center uppercase text-sm hover:text-white',
    ...
    link: 'text-xs uppercase hover:text-amber-50',
    link2: 'hover:text-amber-300 font-bold underline',
  };

  return (
    <Link
      to={to}
      className={`transition-all ease-in-out ${defaultStyles[type]} ${className}`}
      onClick={onClick}
    >
      {children}
    </Link>
